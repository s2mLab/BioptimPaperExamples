...
def generate_table(out):
    ...
    ode_solver_rk4 = OdeSolver.RK4()
    ode_solver_collocation = OdeSolver.COLLOCATION(polynomial_degree=5)

    ocp_euler_RK4 = prepare_ocp(root_folder + "/models/JeChMesh_8DoF.bioMod", final_time=1.5, n_shooting=100, ode_solver=ode_solver_rk4)
    ocp_euler_COLLOCATIONS = prepare_ocp(root_folder + "/models/JeChMesh_8DoF.bioMod", final_time=1.5, n_shooting=100, ode_solver=ode_solver_collocation)
    ocp_quaternion_RK4 = prepare_ocp_quaternion(
        root_folder + "/models/JeChMesh_RootQuat.bioMod", final_time=1.5, n_shooting=100, ode_solver=ode_solver_rk4
    )
    ...
    out.solver[0].ode_solver = ode_solver_rk4
    ...
    out.solver[0].compute_error_single_shooting(sol_euler_RK4)

    out.solver.append(out.Solver("Euler Collocations"))
    out.solver[1].ode_solver = ode_solver_collocation
    ...
    out.solver[1].compute_error_single_shooting(sol_euler_COLLOCATIONS)

    out.solver.append(out.Solver("Quaternion RK4"))
    out.solver[2].ode_solver = ode_solver_rk4
    ...
    out.solver[2].compute_error_single_shooting(sol_quaternion_RK4)
